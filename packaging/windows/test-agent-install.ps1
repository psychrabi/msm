...
Assert-True ($config.PathName -notmatch '--tls-cert|--tls-key') "trusted-LAN service unexpectedly contains TLS arguments"

foreach ($sensitivePath in @($token, $identity)) {
    $acl = Get-Acl -LiteralPath $sensitivePath
    $unexpected = @($acl.Access | Where-Object {
        $_.AccessControlType -eq "Allow" -and
        $_.IdentityReference -notin @("NT AUTHORITY\SYSTEM", "BUILTIN\Administrators")
    })
    Assert-True ($unexpected.Count -eq 0) "sensitive file '$sensitivePath' grants access to unexpected principals"
}
...
$listeners = @(Get-NetTCPConnection -LocalPort (5901..5999) -State Listen -ErrorAction SilentlyContinue)
...
$monitorMetadata = @(Get-ChildItem -LiteralPath $DataDir -Filter "monitors-*.json" -File -ErrorAction SilentlyContinue)
foreach ($file in $monitorMetadata) {
    try { $null = Get-Content -LiteralPath $file.FullName -Raw | ConvertFrom-Json }
    catch { throw "SMOKE TEST FAILED: monitor metadata '$($file.FullName)' is not valid JSON" }
}
...
Write-Host "Monitor metadata files: $($monitorMetadata.Count)"
