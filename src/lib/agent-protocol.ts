...
export type RemoteSession = { sessionId: string; port: number; vncPassword: string };
...
export function normalizeAgentIp(ip: string): string {
  const value = ip.trim();
  if (!value) return "";
  const host =
...
export function isValidAgentIp(ip: string): boolean {
  const value = ip.trim();
...
export function agentId(endpoint: string) {
