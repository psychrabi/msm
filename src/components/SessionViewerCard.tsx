...
const EDGE_BAR_PX = 2,
  EDGE_BAR_LINGER_MS = 1500;

type MonitorTopology = {
  width: number;
  height: number;
  positions: Map<number, CSSProperties>;
};
...
const SessionMonitorPane = memo(function SessionMonitorPane({
  agent,
  session,
  monitor,
  remote,
  ...
  return (
    <div
      className={cn(
        "group/monitor min-h-0 min-w-0 overflow-hidden bg-black",
        paneStyle ? "absolute border border-white/10" : "relative",
      )}
      style={paneStyle ?? { flexGrow: monitorWeight(monitor), flexBasis: 0 }}
    >
      {remote ? (
        <RemoteViewer
          remote={remote}
          ...
          onDisconnect={() =>
            actions.disconnectRemote(agent.id, session.sessionId, monitor.index)
          }
        ...
        <div className="flex h-full min-h-0 w-full items-center justify-center bg-black">
          <Button
            ...
            onClick={() =>
              actions.startSession(agent.id, session.sessionId, monitor.index)
            }
          >
            {connecting ? "Connecting…" : `Connect ${monitor.name}`}
          </Button>
        </div>
      )}
      <div className="pointer-events-none absolute left-2 top-2 z-20 rounded bg-black/65 px-2 py-1 text-[10px] text-white/80 backdrop-blur-sm">
        {monitor.name}
      ...
      {remote && (
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-1.5 top-1.5 z-20 h-7 w-7 bg-black/45 text-white opacity-0 hover:bg-black/70 hover:text-white group-hover/monitor:opacity-100"
          onClick={() =>
            actions.disconnectRemote(agent.id, session.sessionId, monitor.index)
          }
          aria-label={`Disconnect ${monitor.name}`}
        >
...
export const SessionViewerCard = memo(function SessionViewerCard({
  agent,
  session,
  monitors,
  remotes,
  ...
  remotes: Map<number, RemoteConnection>;
  ...
  const totalWeight = useMemo(
    () => monitors.reduce((sum, m) => sum + monitorWeight(m), 0),
    [monitors],
  );
  const viewportRatio = topology
    ? topology.width / topology.height
    : Math.max(totalWeight, 16 / 9);
  const viewportStyle = isFullscreen
    ? undefined
    : { aspectRatio: `${viewportRatio} / 1` };
  const canvasStyle: CSSProperties | undefined = topology
    ? isFullscreen
      ? {
  ...
  const disconnectAll = () => {
    for (const monitor of monitors)
      if (remotes.has(monitor.index))
        actions.disconnectRemote(agent.id, session.sessionId, monitor.index);
    if (isFullscreen) actions.closeFullscreen();
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="min-w-0">
          ...
          <p className="truncate text-[11px] text-white/60">
            ...
            {session.sessionId} · {monitors.length} monitor
            {monitors.length === 1 ? "" : "s"}
            {topology ? " · Windows layout" : ""}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {isFullscreen && connectedCount > 0 && (
          ...
          {connectedCount > 0 && (
            <Button
              ...
              aria-label="Disconnect all monitors"
            >
              <X className="h-4 w-4" />
            </Button>
          )}
      ...
      <CardContent
        className={cn(
          "relative flex min-h-0 items-center justify-center overflow-hidden bg-black p-0",
      ...
      >
        <div
          className={cn(
            topology
              ? "relative shrink-0"
              : "flex h-full min-h-0 w-full divide-x divide-white/10",
          )}
          style={canvasStyle}
        >
          {monitors.map((monitor) => {
            const monitorKey = connectionKey(
              agent.id,
              session.sessionId,
              monitor.index,
            );
            return (
              <SessionMonitorPane
                ...
                session={session}
                monitor={monitor}
                remote={remotes.get(monitor.index)}
                connecting={connectingSessions.has(monitorKey)}
                isFullscreen={isFullscreen}
                fullscreenViewOnly={fullscreenViewOnly}
                paneStyle={topology?.positions.get(monitor.index)}
                actions={actions}
              />
            );
          })}
