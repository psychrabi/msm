...
export function RemoteViewer({
  ...
  const containerRef = useRef<HTMLDivElement>(null),
    rfbRef = useRef<RFB | null>(null),
    disposingRef = useRef(false),
    viewOnlyRef = useRef(viewOnly),
    onDisconnectRef = useRef(onDisconnect),
    onErrorRef = useRef(onError),
    disconnectTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const handlePaste = (event: ClipboardEvent) => {
      ...
      if (!text) return;
      const paste = (rfbRef.current as RfbClipboardApi).clipboardPasteFrom;
      if (!paste) return;
      event.preventDefault();
      paste.call(rfbRef.current as RfbClipboardApi, text);
    };
      ...
      try {
        ...
        rfb.addEventListener("disconnect", (event) => {
          ...
          disconnectTimerRef.current = setTimeout(
            () => {
              disconnectTimerRef.current = null;
              if (!disposingRef.current) onDisconnectRef.current();
            },
            event.detail.clean ? 0 : 500,
          );
        });
        rfbRef.current = rfb;
      } catch {
