...
export function RemoteViewer({ remote, endpoint, token, viewOnly, onDisconnect, onError }: { remote: RemoteConnection; endpoint: string; token: string; viewOnly: boolean; onDisconnect: () => void; onError: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (controlEndpoint.protocol !== "wss:") {
      onErrorRef.current("Secure TLS agent connection is required for remote viewing.");
      return;
    }
    ...
    void (async () => {
      try {
        ...
        if (disposed) return;
        rfb = new RFBClass(container, controlEndpoint.toString(), { credentials: { password: remote.vncPassword } });
        ...
        rfbRef.current = rfb;
      } catch { if (!disposed) onErrorRef.current("Failed to load the VNC viewer component."); }
      finally { if (!disposed) setLoadingModule(false); }
    })();
    return () => {
      disposed = true;
  ...
  }, [remote.agentId, remote.sessionId, remote.vncPassword, remote.vncTicket, endpoint, token]);
