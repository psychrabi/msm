        ...
        <aside className="flex w-64 shrink-0 flex-col border-r bg-muted/20">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {sessions.map((session) => {
              ...
              return <div key={session.sessionId} className="mb-2 rounded-lg border bg-background p-2.5"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-medium">{session.username}</p><p className="text-xs text-muted-foreground">Session {session.sessionId}</p></div><div className="mt-1 flex items-center gap-1.5" title={connecting ? 'Connecting' : connected ? 'Viewer connected' : 'Viewer disconnected'} aria-label={connecting ? 'Connecting' : connected ? 'Viewer connected' : 'Viewer disconnected'}><span className={cn('h-2 w-2 shrink-0 rounded-full', session.state === 'active' ? 'bg-emerald-500' : 'bg-muted-foreground')} />{connecting ? <Activity className="h-3.5 w-3.5 text-muted-foreground" /> : connected ? <Wifi className="h-3.5 w-3.5 text-emerald-500" /> : <WifiOff className="h-3.5 w-3.5 text-muted-foreground" />}</div></div></div>;
            })}
        ...
        <main className="min-w-0 flex-1 overflow-auto">
          <div className="flex min-h-full flex-col">
            ...
            <section className="flex-1 p-5">
              {sessions.length === 0 ? <div className="flex min-h-[360px] items-center justify-center rounded-xl border bg-muted/10 text-center"><div><Monitor className="mx-auto mb-3 h-10 w-10 text-muted-foreground/50" /><p className="text-sm font-medium">No remote sessions available</p><p className="mt-1 text-xs text-muted-foreground">Active Windows sessions will appear here.</p></div></div> : <div className={cn('grid auto-rows-min gap-4', gridColumns(sessions.length))}>
                {sessions.map((session) => {
                  ...
                  return <Card key={session.sessionId} className={cn('flex min-h-0 flex-col overflow-hidden', isFullscreen && 'fixed inset-0 z-50 m-0 rounded-none border-0')}>
                    ...
                    <CardContent className={cn('relative min-h-0 bg-black p-0 viewer-viewport', isFullscreen && 'rounded-none viewer-fullscreen')}>
                      {remote ? <RemoteViewer remote={remote} endpoint={endpoint} token={token} viewOnly={isFullscreen ? fullscreenViewOnly : true} onDisconnect={() => disconnectRemote(session.sessionId)} onError={setError} /> : <div className="flex h-full w-full items-center justify-center bg-black"><Button size="sm" disabled={!socket || connecting} onClick={() => void startRemoteSession(session.sessionId)}>{connecting ? 'Connecting…' : 'Connect'}</Button></div>}
                    </CardContent>
