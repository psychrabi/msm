...
type AgentConnection = { id: string; endpoint: string; token: string; identity: DeviceIdentity | null; sessions: Session[]; status: AgentStatus; error: string; remembered: boolean };
...
function loadSavedAgents(): SavedAgent[] {
  try {
    ...
    if (raw) { const parsed = JSON.parse(raw) as unknown; if (Array.isArray(parsed)) return parsed.filter((item): item is SavedAgent => Boolean(item && typeof item === 'object' && typeof (item as SavedAgent).endpoint === 'string')).map((item) => ({ endpoint: normalizeEndpoint(item.endpoint) })); }
...
function addSavedAgent(endpoint: string) { const normalized = normalizeEndpoint(endpoint); saveSavedAgents([...loadSavedAgents().filter((agent) => agent.endpoint !== normalized), { endpoint: normalized }]); }
...
export default function MultiAgentApp() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (legacyEndpoint && !localStorage.getItem(SAVED_AGENTS_KEY)) saved = [{ endpoint: normalizeEndpoint(legacyEndpoint) }];
      const loaded: AgentConnection[] = [];
      for (const item of saved) {
        try {
          let token = await getCredential(item.endpoint);
          if (!token && legacyToken && legacyEndpoint && normalizeEndpoint(legacyEndpoint) === item.endpoint) { await setCredential(item.endpoint, legacyToken); token = legacyToken; }
        ...
        } catch (error) { loaded.push({ id: agentId(item.endpoint), endpoint: item.endpoint, token: '', identity: null, sessions: [], status: 'Disconnected', error: String(error), remembered: true }); }
      }
      if (legacyEndpoint) { localStorage.removeItem(LEGACY_ENDPOINT_KEY); localStorage.removeItem(LEGACY_TOKEN_KEY); }
      if (cancelled) return;
      setAgents(loaded);
      for (const agent of loaded) if (agent.token) setTimeout(() => void connectAgentRef.current(agent.id, true), 0);
    })();
  ...
  function scheduleReconnect(id: string) { const agent = agentsRef.current.find((item) => item.id === id); if (!agent || !agent.remembered || !agent.token || manualDisconnectRef.current.has(id) || reconnectTimersRef.current.has(id)) return; updateAgent(id, { status: 'Reconnecting…' }); reconnectTimersRef.current.set(id, setTimeout(() => { reconnectTimersRef.current.delete(id); void connectAgent(id, true); }, RECONNECT_DELAY_MS)); }
  async function refreshSessions(id: string) { const socket = socketsRef.current.get(id); if (!socket) return; try { await socket.send(JSON.stringify({ type: 'listSessions' })); } catch { await disconnectAgentSocket(id); scheduleReconnect(id); } }
  ...
  async function disconnectAgent(id: string) {
    manualDisconnectRef.current.add(id); clearReconnectTimer(id);
    pendingRemoteRequestsRef.current.forEach((key) => { if (key.startsWith(`${id}::`)) pendingRemoteRequestsRef.current.delete(key); });
  ...
  async function removeAgent(id: string) { const agent = agentsRef.current.find((item) => item.id === id); if (!agent) return; await disconnectAgent(id); await deleteCredential(agent.endpoint).catch(() => undefined); removeSavedAgent(agent.endpoint); setAgents((current) => current.filter((item) => item.id !== id)); }
  ...
  useEffect(() => { const interval = setInterval(() => { for (const agent of agentsRef.current) if (socketsRef.current.has(agent.id)) void refreshSessions(agent.id); }, HEALTH_CHECK_INTERVAL_MS); return () => clearInterval(interval); }, []);
      ...
      <aside className="flex w-72 shrink-0 flex-col border-r bg-muted/20">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {agents.map((agent) => <div key={agent.id} className="mb-3 rounded-lg border bg-background p-3">
            ...
            <div className="mt-2 flex gap-1"><Button className="flex-1" size="sm" variant="outline" disabled={agent.status === 'Connected' || agent.status === 'Connecting…' || agent.status === 'Reconnecting…' || !agent.token} onClick={() => void connectAgent(agent.id)}>{agent.status === 'Reconnecting…' ? 'Reconnecting…' : 'Connect'}</Button><Button size="sm" variant="ghost" disabled={agent.status === 'Disconnected'} onClick={() => void disconnectAgent(agent.id)}>Disconnect</Button><Button size="sm" variant="ghost" onClick={() => void removeAgent(agent.id)}><X className="h-4 w-4" /></Button></div>
