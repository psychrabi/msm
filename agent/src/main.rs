...
use axum::{extract::{ws::{Message, WebSocket, WebSocketUpgrade}, Path, Query, State}, http::{header::AUTHORIZATION, HeaderMap, StatusCode}, response::IntoResponse, routing::{any, get}, Json, Router};
use clap::Parser;
...
struct Args { #[arg(long, default_value=DEFAULT_LISTEN)] listen: SocketAddr, #[arg(long)] print_identity: bool }
...
#[serde(tag="type", rename_all="camelCase")]
enum ClientMessage { ListSessions, StartSession { session_id: String }, Ping }
...
async fn start_session(state:&AppState,session_id:&str)->Result<RemoteSession,Box<dyn std::error::Error+Send+Sync>>{let id:u32=session_id.parse()?;if let Some(existing)=state.workers.lock().await.get(&id).cloned(){return Ok(existing)}let port=FIRST_VNC_PORT+(id%1000)as u16;let password=state.auth_token.chars().take(8).collect::<String>();spawn_worker(id,port,&password)?;let session=RemoteSession{session_id:session_id.to_owned(),port,vnc_password:password};state.workers.lock().await.insert(id,session.clone());Ok(session)}
#[cfg(windows)]
fn spawn_worker(session_id:u32,port:u16,password:&str)->Result<(),Box<dyn std::error::Error+Send+Sync>>{use std::{ffi::OsStr,os::windows::ffi::OsStrExt,process::Command};use windows::{core::PWSTR,Win32::Foundation::CloseHandle,Win32::System::RemoteDesktop::WTSQueryUserToken,Win32::System::Threading::{CreateProcessAsUserW,PROCESS_CREATION_FLAGS,PROCESS_INFORMATION,STARTUPINFOW,ProcessIdToSessionId,GetCurrentProcessId}};unsafe{let mut token=Default::default();if WTSQueryUserToken(session_id,&mut token).is_ok(){let exe=env::current_exe()?.with_file_name("msm-agent-worker.exe");let command=format!("\"{}\" --session-id {} --port {} --password {}",exe.display(),session_id,port,password);let mut command_w:Vec<u16>=OsStr::new(&command).encode_wide().chain(Some(0)).collect();let desktop:Vec<u16>=OsStr::new("winsta0\\default").encode_wide().chain(Some(0)).collect();let mut startup=STARTUPINFOW::default();startup.cb=std::mem::size_of::<STARTUPINFOW>()as u32;startup.lpDesktop=PWSTR(desktop.as_ptr()as*mut u16);let mut process=PROCESS_INFORMATION::default();CreateProcessAsUserW(Some(token),None,Some(PWSTR(command_w.as_mut_ptr())),None,None,false,PROCESS_CREATION_FLAGS(0),None,None,&startup,&mut process)?;CloseHandle(process.hThread)?;CloseHandle(process.hProcess)?;CloseHandle(token)?;return Ok(())}let mut current_session=0u32;ProcessIdToSessionId(GetCurrentProcessId(),&mut current_session)?;if current_session==session_id{let exe=env::current_exe()?.with_file_name("msm-agent-worker.exe");Command::new(exe).args(["--session-id",&session_id.to_string(),"--port",&port.to_string(),"--password",password]).spawn()?;return Ok(())}Err("unable to obtain the target Windows session token; run the agent as LocalSystem for multiseat service mode".into())}}
#[cfg(not(windows))]
fn spawn_worker(_session_id:u32,_port:u16,_password:&str)->Result<(),Box<dyn std::error::Error+Send+Sync>>{Err("Windows only".into())}
#[tokio::main]
async fn main()->Result<(),Box<dyn std::error::Error>>{tracing_subscriber::fmt::init();let args=Args::parse();let identity=load_or_create_identity()?;let auth_token=load_or_create_token()?;if args.print_identity{println!("{}",serde_json::to_string_pretty(&identity)?);println!("access_token={auth_token}");return Ok(())}let state=AppState{identity:identity.clone(),auth_token,workers:Arc::new(Mutex::new(HashMap::new()))};let app=Router::new().route("/health",get(health)).route("/ws",any(websocket)).route("/vnc/{session_id}",any(vnc_websocket)).with_state(state);let listener=tokio::net::TcpListener::bind(args.listen).await?;info!(device_id=%identity.device_id,device_name=%identity.device_name,listen=%args.listen,"MSM Windows agent started");axum::serve(listener,app).with_graceful_shutdown(async{let _=signal::ctrl_c().await;warn!("shutdown requested")}).await?;Ok(())}
