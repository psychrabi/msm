...
#[cfg(windows)]
fn query_username(session_id: u32) -> Option<String> { use windows::Win32::System::RemoteDesktop::{WTS_CURRENT_SERVER_HANDLE, WTSFreeMemory, WTSQuerySessionInformationW, WTSUserName}; unsafe { let mut buffer = windows::core::PWSTR::null(); let mut bytes = 0u32; WTSQuerySessionInformationW(Some(WTS_CURRENT_SERVER_HANDLE), session_id, WTSUserName, &mut buffer, &mut bytes).ok()?; let chars = std::slice::from_raw_parts(buffer.as_ptr(), (bytes as usize / 2).saturating_sub(1)); let name = String::from_utf16_lossy(chars); WTSFreeMemory(buffer.as_ptr() as _); Some(name) } }
#[cfg(not(windows))]
