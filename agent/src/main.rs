...
async fn start_session(
    ...
    if let Some(existing) = state.workers.lock().await.get(&id).cloned() {
        return Ok(existing);
    }
    ...
    Err(format!("VNC worker for session {id} did not become ready").into())
}
#[cfg(windows)]
fn spawn_worker(
    session_id: u32,
...
fn run_as_windows_service() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn service_main(_arguments: Vec<OsString>) {
        ...
        let status_handle =
            match service_control_handler::register("MSMAgent", move |event| match event {
