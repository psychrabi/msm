...
async fn build_app() -> Result<(Router, DeviceIdentity), Box<dyn std::error::Error + Send + Sync>> { let identity = load_or_create_identity()?; let auth_token = load_or_create_token()?; let state = AppState { identity: identity.clone(), auth_token, workers: Arc::new(Mutex::new(HashMap::new())) }; let app = Router::new().route("/health", get(health)).route("/ws", any(websocket)).route("/vnc/{session_id}", any(vnc_websocket)).with_state(state); Ok((app, identity)) }
async fn run_server(listen: SocketAddr, shutdown: impl std::future::Future<Output = ()> + Send + 'static, ready: Option<oneshot::Sender<Result<(), String>>>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> { let (app, identity) = build_app().await?; let listener = match tokio::net::TcpListener::bind(listen).await { Ok(listener) => listener, Err(error) => { if let Some(tx) = ready { let _ = tx.send(Err(error.to_string())); } return Err(error.into()); } }; if let Some(tx) = ready { let _ = tx.send(Ok(())); } info!(device_id=%identity.device_id, device_name=%identity.device_name, listen=%listen, "MSM Windows agent started"); axum::serve(listener, app).with_graceful_shutdown(shutdown).await?; Ok(()) }

#[cfg(windows)] const SERVICE_NAME: &str = "MSMAgent";
#[cfg(windows)] const SERVICE_DISPLAY_NAME: &str = "MSM Agent";
#[cfg(windows)] const SERVICE_DESCRIPTION: &str = "MSM multiseat remote monitor and control agent";
#[cfg(windows)] const SERVICE_LISTEN: &str = "0.0.0.0:40123";

#[cfg(windows)]
fn install_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { use std::{ffi::OsString, path::PathBuf}; use windows_service::{ service::{ServiceAccess, ServiceErrorControl, ServiceInfo, ServiceStartType, ServiceType}, service_manager::{ServiceManager, ServiceManagerAccess} }; let service_manager = ServiceManager::local_computer(None::<&str>, ServiceManagerAccess::CONNECT | ServiceManagerAccess::CREATE_SERVICE)?; let executable_path = env::current_exe()?; let service_info = ServiceInfo { name: OsString::from(SERVICE_NAME), display_name: OsString::from(SERVICE_DISPLAY_NAME), service_type: ServiceType::OWN_PROCESS, start_type: ServiceStartType::AutoStart, error_control: ServiceErrorControl::Normal, executable_path: PathBuf::from(&executable_path), launch_arguments: vec![OsString::from("--run-service")], dependencies: vec![], account_name: None, account_password: None }; let service_access = ServiceAccess::QUERY_STATUS | ServiceAccess::QUERY_CONFIG | ServiceAccess::CHANGE_CONFIG | ServiceAccess::START | ServiceAccess::STOP | ServiceAccess::DELETE; if let Ok(existing) = service_manager.open_service(SERVICE_NAME, service_access) { let _ = existing.stop(); let _ = existing.delete(); drop(existing); std::thread::sleep(Duration::from_millis(500)); } let service = service_manager.create_service(&service_info, service_access)?; service.set_description(SERVICE_DESCRIPTION)?; println!("MSM Agent service installed."); println!("Executable: {}", executable_path.display()); println!("Service arguments: --run-service"); Ok(()) }
#[cfg(windows)]
fn uninstall_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { use windows_service::{service::{ServiceAccess, ServiceState}, service_manager::{ServiceManager, ServiceManagerAccess}}; let service_manager = ServiceManager::local_computer(None::<&str>, ServiceManagerAccess::CONNECT)?; let service_access = ServiceAccess::QUERY_STATUS | ServiceAccess::STOP | ServiceAccess::DELETE; let service = match service_manager.open_service(SERVICE_NAME, service_access) { Ok(service) => service, Err(error) => { println!("MSM Agent service is not installed: {error}"); return Ok(()); } }; if service.query_status()?.current_state != ServiceState::Stopped { let _ = service.stop(); } service.delete()?; println!("MSM Agent service uninstalled."); Ok(()) }
#[cfg(windows)]
fn start_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { use windows_service::{service::ServiceAccess, service_manager::{ServiceManager, ServiceManagerAccess}}; let service_manager = ServiceManager::local_computer(None::<&str>, ServiceManagerAccess::CONNECT)?; let service = service_manager.open_service(SERVICE_NAME, ServiceAccess::START | ServiceAccess::QUERY_STATUS)?; service.start::<&str>(&[])?; println!("MSM Agent service start requested."); Ok(()) }
#[cfg(windows)]
fn stop_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { use windows_service::{service::ServiceAccess, service_manager::{ServiceManager, ServiceManagerAccess}}; let service_manager = ServiceManager::local_computer(None::<&str>, ServiceManagerAccess::CONNECT)?; let service = service_manager.open_service(SERVICE_NAME, ServiceAccess::STOP | ServiceAccess::QUERY_STATUS)?; let _ = service.stop()?; println!("MSM Agent service stop requested."); Ok(()) }

#[cfg(windows)]
fn run_as_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { use std::ffi::OsString; use std::sync::mpsc; use windows_service::{define_windows_service, service::{ServiceControl, ServiceControlAccept, ServiceExitCode, ServiceState, ServiceStatus, ServiceType}, service_control_handler::{self, ServiceControlHandlerResult}, service_dispatcher}; define_windows_service!(ffi_service_main, service_main); fn service_main(_arguments: Vec<OsString>) { let (stop_tx, stop_rx) = mpsc::channel::<()>(); let status_handle = match service_control_handler::register(SERVICE_NAME, move |event| match event { ServiceControl::Stop | ServiceControl::Shutdown => { let _ = stop_tx.send(()); ServiceControlHandlerResult::NoError } _ => ServiceControlHandlerResult::NoError }) { Ok(handle) => handle, Err(error) => { eprintln!("failed to register MSM service control handler: {error}"); return; } }; let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::StartPending, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(0), checkpoint: 1, wait_hint: Duration::from_secs(15), process_id: None }); let runtime = match tokio::runtime::Builder::new_multi_thread().enable_all().build() { Ok(runtime) => runtime, Err(error) => { eprintln!("failed to create Tokio runtime for MSM service: {error}"); let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Stopped, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(1), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); return; } }; let (result_tx, result_rx) = oneshot::channel::<Result<(), String>>(); let (stop_tx_async, stop_rx_async) = oneshot::channel::<()>(); std::thread::spawn(move || { let _ = stop_rx.recv(); let _ = stop_tx_async.send(()); }); let server = runtime.spawn(run_server(SERVICE_LISTEN.parse().expect("valid service listen address"), async move { let _ = stop_rx_async.await; }, Some(result_tx))); match result_rx.blocking_recv() { Ok(Ok(())) => { let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Running, controls_accepted: ServiceControlAccept::STOP | ServiceControlAccept::SHUTDOWN, exit_code: ServiceExitCode::Win32(0), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); } Ok(Err(error)) => { eprintln!("MSM service failed to start: {error}"); let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Stopped, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(1), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); return; } Err(error) => { eprintln!("MSM service startup channel failed: {error}"); let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Stopped, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(1), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); return; } } let result = match runtime.block_on(server) { Ok(result) => result, Err(error) => Err(Box::new(error) as Box<dyn std::error::Error + Send + Sync>), }; let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Stopped, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(if result.is_ok() { 0 } else { 1 }), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); } service_dispatcher::start(SERVICE_NAME, ffi_service_main)?; Ok(()) }
#[cfg(windows)] fn run_service_entrypoint() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { run_as_windows_service() }
fn run_normal(args: Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> { let runtime = tokio::runtime::Builder::new_multi_thread().enable_all().build()?; runtime.block_on(run_server(args.listen, async { let _ = signal::ctrl_c().await; warn!("shutdown requested") }, None)) }
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { tracing_subscriber::fmt::init(); let args = Args::parse(); if args.print_identity { let identity = load_or_create_identity()?; let auth_token = load_or_create_token()?; println!("{}", serde_json::to_string_pretty(&identity)?); println!("access_token={auth_token}"); return Ok(()); } #[cfg(windows)] { if args.install_service { return install_windows_service(); } if args.uninstall_service { return uninstall_windows_service(); } if args.start_service { return start_windows_service(); } if args.stop_service { return stop_windows_service(); } if args.run_service { return run_service_entrypoint(); } } #[cfg(not(windows))] if args.install_service || args.uninstall_service || args.start_service || args.stop_service || args.run_service { return Err("Windows service management is only supported on Windows".into()); } run_normal(args) }
