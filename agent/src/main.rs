...
use tokio::{io::{AsyncReadExt, AsyncWriteExt}, net::TcpStream, signal, sync::Mutex, time::{sleep, Duration}};
...
#[cfg(windows)]fn run_as_windows_service()->Result<(),Box<dyn std::error::Error>>{
    ...
    fn service_main(_arguments: Vec<OsString>) {
        ...
        let result = runtime.block_on(async move {
            ...
            run_server(DEFAULT_LISTEN.parse().expect("valid default listen"), async move {
...
async fn main()->Result<(),Box<dyn std::error::Error>>{
    ...
    #[cfg(windows)] if args.service { return run_as_windows_service(); }
    #[cfg(not(windows))] if args.service { return Err("--service is only supported on Windows".into()); }
    run_server(args.listen, async { let _=signal::ctrl_c().await; warn!("shutdown requested"); }).await
}
