...
async fn vnc_websocket(Path(session_id): Path<u32>, Query(query): Query<TokenQuery>, State(state): State<AppState>, ws: WebSocketUpgrade) -> impl IntoResponse {
...
fn windows_sessions() -> Result<Vec<SessionInfo>, Box<dyn std::error::Error + Send + Sync>> {
    use windows::Win32::System::RemoteDesktop::{WTS_CURRENT_SERVER_HANDLE, WTS_SESSION_INFOW, WTSActive, WTSFreeMemory, WTSEnumerateSessionsW};
    unsafe {
...
fn query_username(session_id: u32) -> Option<String> {
    use windows::{core::PWSTR, Win32::System::RemoteDesktop::{WTS_CURRENT_SERVER_HANDLE, WTSFreeMemory, WTSQuerySessionInformationW, WTSUserName}};
    unsafe {
        let mut buffer = PWSTR(std::ptr::null_mut());
        let mut bytes = 0u32;
        WTSQuerySessionInformationW(Some(WTS_CURRENT_SERVER_HANDLE), session_id, WTSUserName, &mut buffer, &mut bytes).ok()?;
...
fn spawn_worker(session_id: u32, port: u16, password: &str) -> Result<u32, Box<dyn std::error::Error + Send + Sync>> {
    ...
    unsafe {
        ...
        CreateProcessAsUserW(Some(token), None, Some(PWSTR(command_w.as_mut_ptr())), None, None, false, PROCESS_CREATION_FLAGS(0), None, None, &startup, &mut process).map_err(|e| format!("CreateProcessAsUserW(session {session_id}) failed: {e}"))?;
...
fn terminate_worker(pid: u32) {
    ...
    unsafe {
        match OpenProcess(PROCESS_TERMINATE, false, pid) {
            Ok(process) => {
                if let Err(error) = TerminateProcess(process, 1) { warn!(pid, %error, "failed to terminate VNC worker"); }
...
async fn run_server(listen: SocketAddr, shutdown: impl std::future::Future<Output = ()> + Send + 'static, ready: Option<oneshot::Sender<Result<(), String>>>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let listener = match tokio::net::TcpListener::bind(listen).await {
        ...
        Err(error) => {
            if let Some(tx) = ready { let _ = tx.send(Err(error.to_string())); }
...
fn install_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if let Ok(existing) = service_manager.open_service(SERVICE_NAME, service_access) {
        let _ = existing.stop(); let _ = existing.delete(); drop(existing); std::thread::sleep(Duration::from_millis(500));
    }
...
fn run_as_windows_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    fn service_main(_arguments: Vec<OsString>) {
        ...
        let status_handle = match service_control_handler::register(SERVICE_NAME, move |event| match event {
            ServiceControl::Stop | ServiceControl::Shutdown => { let _ = stop_tx.send(()); ServiceControlHandlerResult::NoError }
        ...
        let runtime = match tokio::runtime::Builder::new_multi_thread().enable_all().build() {
            Ok(runtime) => runtime,
            Err(error) => { eprintln!("failed to create Tokio runtime for MSM service: {error}"); let _ = status_handle.set_service_status(ServiceStatus { service_type: ServiceType::OWN_PROCESS, current_state: ServiceState::Stopped, controls_accepted: ServiceControlAccept::empty(), exit_code: ServiceExitCode::Win32(1), checkpoint: 0, wait_hint: Duration::default(), process_id: None }); return; }
        };
...
#[cfg(windows)]
fn run_service_entrypoint() -> Result<(), Box<dyn std::error::Error + Send + Sync>> { run_as_windows_service() }

fn run_normal(args: Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let runtime = tokio::runtime::Builder::new_multi_thread().enable_all().build()?;
    runtime.block_on(run_server(args.listen, async { let _ = signal::ctrl_c().await; warn!("shutdown requested") }, None))
}
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    #[cfg(not(windows))]
    if args.install_service || args.uninstall_service || args.start_service || args.stop_service || args.run_service {
