    ...
    monitors.sort_by(|a, b| {
        ...
        bp.cmp(&ap)
            ...
            .then_with(|| {
                a.friendly_name()
                    .unwrap_or_default()
                    .cmp(&b.friendly_name().unwrap_or_default())
            })
    });
    Ok(monitors)
}
    ...
    if let Some(parent) = path.parent() {
        if let Err(error) = fs::create_dir_all(parent) {
            warn!(
                session_id,
                ?error,
                "unable to create monitor metadata directory"
            );
            return;
        }
    }
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (server, mut events) = VncServer::new(
        width,
        height,
        format!("MSM Session {} - {}", args.session_id, monitor_name),
    ...
    std::thread::spawn(move || {
        ...
        let Some(monitor) = monitors.into_iter().nth(monitor_index) else {
            error!(
                session_id,
                monitor_index, "selected monitor disappeared before capture started"
            );
            return;
        };
        loop {
    ...
    unsafe {
        ...
        let result = if IsClipboardFormatAvailable(format).is_ok() {
            ...
            if ptr.is_null() || size == 0 {
                ...
                let max = (size as usize) / 2;
                let slice = std::slice::from_raw_parts(ptr as *const u16, max);
                let len = slice.iter().position(|v| *v == 0).unwrap_or(max);
...
fn set_windows_clipboard(text: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    unsafe {
        ...
        let memory = match GlobalAlloc(GMEM_MOVEABLE, wide.len() * 2) {
            Ok(v) => v,
            Err(e) => {
                let _ = CloseClipboard();
                return Err(e.into());
...
#[cfg(not(target_os = "windows"))]
fn set_windows_clipboard(_: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
...
fn apply_button_transitions(previous_mask: u8, current_mask: u8) {
    ...
    for (bit, down, up, name) in [
        ...
        let was = previous_mask & bit != 0;
        let is = current_mask & bit != 0;
        if was == is {
            continue;
        }
        unsafe { mouse_event(if is { down } else { up }, 0, 0, 0, 0) }
        info!(
            previous_mask,
            current_mask,
            button = name,
