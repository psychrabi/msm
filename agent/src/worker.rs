...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let monitor = monitors.iter().find(|m| m.is_primary().unwrap_or(false)).cloned()
...
fn apply_buttons(enigo: &mut Enigo, mask: u8) {
    for (bit, button) in [(1u8, Button::Left), (2u8, Button::Middle), (4u8, Button::Right)] {
        let direction = if mask & bit != 0 { Direction::Press } else { Direction::Release };
...
fn map_keysym(key: u32) -> Option<Key> {
    let mapped = match key {
        0xff08 => Key::Backspace, 0xff09 => Key::Tab, 0xff0d => Key::Return, 0xff1b => Key::Escape,
        0xff50 => Key::Home, 0xff51 => Key::LeftArrow, 0xff52 => Key::UpArrow, 0xff53 => Key::RightArrow,
        0xff54 => Key::DownArrow, 0xff55 => Key::PageUp, 0xff56 => Key::PageDown, 0xff57 => Key::End,
        ...
        0xffe1 => Key::Shift, 0xffe2 => Key::RShift, 0xffe3 => Key::Control, 0xffe4 => Key::RControl,
        0xffe9 => Key::Alt, 0xffea => Key::Alt, 0xffeb => Key::Meta, 0xffec => Key::Meta,
        0x20 => Key::Space, 0x21..=0x7e => Key::Unicode(char::from_u32(key)?), _ => return None,
    };
    Some(mapped)
}
