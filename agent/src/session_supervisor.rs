use crate::{
    AppState, FIRST_VNC_PORT, MAX_VNC_PORT, RemoteSession, WorkerKey, spawn_worker, terminate_worker,
};
...
fn reconcile(state: &AppState) {
    let sessions = match enumerate_windows_sessions() { Ok(s) => s, Err(e) => { warn!(%e,"failed to enumerate Windows sessions; preserving workers"); return; } };
    let active_sessions: HashSet<u32> = sessions.iter().map(|s| s.session_id).collect();
    {
        ...
        workers.retain(|key, worker| {
            if is_process_alive(worker.worker_pid) { true } else { warn!(session_id=key.session_id, monitor_index=key.monitor_index, worker_pid=worker.worker_pid, "worker exited"); false }
        });
    }
    {
        let mut workers = state.workers.blocking_lock();
        let stale: Vec<(WorkerKey,u32)> = workers.iter().filter_map(|(key,w)| (!active_sessions.contains(&key.session_id)).then_some((*key,w.worker_pid))).collect();
        for (key,pid) in stale { info!(session_id=key.session_id, monitor_index=key.monitor_index, worker_pid=pid,"interactive session ended; stopping worker"); terminate_worker(pid); workers.remove(&key); clear_failure(state,key.session_id); }
    }
    ...
    for session in sessions {
        let key = WorkerKey { session_id: session.session_id, monitor_index: 0 };
        let alive = state.workers.blocking_lock().get(&key).is_some_and(|w| is_process_alive(w.worker_pid));
        if alive { clear_failure(state, session.session_id); continue; }
        if !retry_allowed(state, session.session_id) { continue; }
        match spawn_worker_for_monitor(state, &session, 0, true) {
            Ok(_) => clear_failure(state, session.session_id),
            Err(e) => { record_failure(state, session.session_id); warn!(session_id=session.session_id,username=%session.username,%e,"failed to start primary worker; watchdog will retry"); }
        }
    }
}

fn spawn_worker_for_monitor(state: &AppState, session: &SessionInfo, monitor_index: u32, legacy_primary: bool) -> Result<RemoteSession, Box<dyn std::error::Error + Send + Sync>> {
    let _lifecycle_lock = state.worker_operations.blocking_lock();
    let key = WorkerKey { session_id: session.session_id, monitor_index };
    if let Some(existing) = state.workers.blocking_lock().get(&key).cloned() {
        if is_process_alive(existing.worker_pid) { return Ok(existing); }
        terminate_worker(existing.worker_pid); state.workers.blocking_lock().remove(&key);
    }
    let port = ({ let workers=state.workers.blocking_lock(); allocate_worker_port(&workers) }).ok_or_else(|| "no available VNC worker ports".to_owned())?;
    let password = uuid::Uuid::new_v4().simple().to_string();
    let worker_pid = spawn_worker(session.session_id, port, &password, if legacy_primary { None } else { Some(monitor_index) })?;
    ...
    if !wait_for_worker_ready(worker_pid, port) { terminate_worker(worker_pid); return Err(format!("worker PID {worker_pid} did not become ready on port {port}").into()); }
    let remote = RemoteSession { session_id: session.session_id.to_string(), monitor_index, port, vnc_password: password, vnc_ticket: String::new(), worker_pid };
    state.workers.blocking_lock().insert(key, remote.clone()); Ok(remote)
}
...
fn is_process_alive(pid:u32)->bool{
    ...
    #[cfg(not(windows))] { let _=pid; true }
}

#[cfg(test)] mod tests { use super::*; fn worker(port:u16,session_id:u32,monitor_index:u32)->RemoteSession{RemoteSession{session_id:session_id.to_string(),monitor_index,port,vnc_password:"test".into(),vnc_ticket:String::new(),worker_pid:1}} #[test] fn retry_is_bounded(){assert_eq!(retry_delay(1),Duration::from_secs(3));assert_eq!(retry_delay(6),MAX_RETRY_DELAY)} #[test] fn allocates_ports_across_monitors(){let mut workers=std::collections::HashMap::new();workers.insert(WorkerKey{session_id:1,monitor_index:0},worker(FIRST_VNC_PORT,1,0));workers.insert(WorkerKey{session_id:1,monitor_index:1},worker(FIRST_VNC_PORT+1,1,1));assert_eq!(allocate_worker_port(&workers),Some(FIRST_VNC_PORT+2));} }
